...
We extend \citet{Brown2021Memorization} to a partitioned dataset. There are \(K\ge 2\) clients. For \(P\sim q\), client \(k\) holds local dataset \(X_k\); we write \(X=(X_1,\dots,X_K)\) and \(X_{-k}\) for everything except client~\(k\). One round of DP-FedAvg, parametrized by the clip bound \(C>0\) and the noise scale \(\sigma>0\), proceeds as follows.
...
\begin{assumption}[Conditional independence given \(P\)]\label{ass:indep}
Conditional on \(P\), the client datasets \(X_1,\dots,X_K\) are mutually independent. The client randomness \((R_1,\dots,R_K)\) is mutually independent and independent of \((X_1,\dots,X_K)\) and of \(P\). The server noise \(Z\) is drawn fresh, independent of \((X,R_1,\dots,R_K,P)\).
\end{assumption}

Assumption~\ref{ass:indep} matches the Brown et al.\ generative model (i.i.d.\ examples from \(P\)) and is the natural extension to a partitioned dataset. It is the same independence model used implicitly in standard DP-FedAvg analyses.
