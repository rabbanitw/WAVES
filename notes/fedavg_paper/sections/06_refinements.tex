...
\subsection{Sharper bound for stochastic-gradient clients}\label{sec:non-iid}

The variance-form bound of Theorem~\ref{thm:var-pp} is generic: it works for any clip-bounded client map \(V_k^{(t)}\). When \(V_k^{(t)}\) is in fact a stochastic-gradient step---as it is in essentially every DP-FedAvg deployment---the standard FL stochastic-gradient variance assumption \citep[Assumption~3]{Bornstein2022SWIFT} controls the conditional update variance directly, yielding a bound in terms of the algorithmic constants \((\eta,\sigma_g^2)\) rather than the worst-case clip \(C\). The improvement is strict whenever \(\eta^2\sigma_g^2<C^2\) (the typical regime once the iterate is past the high-gradient phase). The inter-client gradient heterogeneity constant \(\zeta^2\) of \citet[Assumption~3]{Bornstein2022SWIFT}, which dominates FL utility bounds, does not appear in the result---per-client memorization is heterogeneity-independent because we condition on the full instance \(P=(P_1,\dots,P_K)\) of Assumption~\ref{ass:indep}.

\paragraph{Client update model.} We restrict the per-round client update to be a single stochastic-gradient step:
...
I(X_k;\transcript\mid P) \;\le\; \sum_{t=0}^{T-1}\frac{q\,\eta^2\sigma_g^2}{2K^2\sigma_t^2}\;=\;\frac{qT\eta^2\sigma_g^2}{2K^2\sigma^2}\quad(\sigma_t\equiv\sigma).
\end{equation}
...
\begin{proof}
We carry the proof of Theorem~\ref{thm:var-pp} through with \(P\) in place of \(P\); the only modification is in bounding the per-round conditional update variance.

\textbf{Step A--B.} Steps A and B of the proof of Theorem~\ref{thm:multi-round} go through verbatim with \(P\) replaced by \(P\): conditional independence \(X_k\indep X_{-k}\mid P\) is the very definition of the non-IID release model, so
\[
I(X_k;\transcript\mid P)\;\le\;I(X_k;\transcript\mid P,X_{-k})\;=\;\sum_{t=0}^{T-1}I\!\left(X_k;\,\noised^{(t)}\,\middle|\,P,X_{-k},\hist^{(t)}\right).
\]

\textbf{Step C', C''.} Steps C' and C'' of the proof of Theorem~\ref{thm:var-pp} likewise go through with \(P\) in place of \(P\), yielding
\begin{equation}\label{eq:non-iid-pre}
I(X_k;\transcript\mid P)\;\le\;\sum_{t=0}^{T-1}\frac{q\,v_k^{(t)}}{2K^2\sigma_t^2},
\qquad
v_k^{(t)}\;:=\;\E\!\left[\tr\Cov\!\left(U_k^{(t)}\,\Big|\,P,X_{-k},\hist^{(t)}\right)\right].
\end{equation}

\textbf{Step E: bound \(v_k^{(t)}\) by \(\eta^2\sigma_g^2\).}
Fix \(t\) and condition on \(\mathcal C_t:=(P,X_{-k},\hist^{(t)})\). Under \(\mathcal C_t\):
(i) \(\theta^{(t)}\) is a deterministic function of \(\hist^{(t)}\subseteq\mathcal C_t\) and is therefore fixed;
(ii) \(P_k\) is fixed (it is a component of \(P\subseteq\mathcal C_t\)), so \(\nabla f_k(\theta^{(t)})\) is a deterministic function of \(\mathcal C_t\);
...
If \(\xi_k^{(t)}\) is a mini-batch of \(m\) iid draws from \(P_k\) and \(g\) is the mini-batch average, then \(\E[\|g(\theta;\xi)-\nabla f_k(\theta)\|^2\mid P_k]\le\sigma_g^2/m\), and the bound improves to \(I(X_k;\transcript\mid P)\le qT\eta^2\sigma_g^2/(2K^2 m\sigma^2)\).
\end{remark}
