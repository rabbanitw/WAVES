...
\paragraph{Federated partition.} For each \(K\in\{1,2,4,6,8,10\}\) we partition the long-tailed dataset across clients with a Dirichlet distribution: for each class \(c\), draw \(p_c\sim\mathrm{Dirichlet}(\alpha,\dots,\alpha)\in\Delta^{K-1}\) with \(\alpha=0.5\) and split that class's training and test indices across clients in proportion \(p_c\). The per-class proportions are shared between train and test, so each client receives a (train\(_k\), test\(_k\)) pair drawn from the same client-specific class mixture. This is the non-IID instance of Assumption~\ref{ass:indep}: the joint instance \(P=(P_1,\dots,P_K)\) is realized as a tuple of distinct per-client mixtures over the 50 class-conditional distributions, with mixture weights given by the Dirichlet draws. Inter-client heterogeneity is present by construction---at \(K=10\), most clients see only a handful of classes with appreciable mass---rather than imposed as a perturbation on a common \(P\). Strictly speaking, the partition is the standard finite-sample proxy for fresh i.i.d.\ draws from each \(P_k\): client subsets are disjoint rather than independent, so the conditional independence in Assumption~\ref{ass:indep} holds only asymptotically (as the underlying pool grows for fixed Dirichlet draws). This finite-sample gap is common to essentially all empirical FL benchmarks built on Dirichlet partitions and is not specific to our experiment.
