"""Batch symmetric diffusive regeneration: apply N denoising-iteration regen
to every image in --in-dir, writing one PNG per input to --out-dir.

Same regen algorithm as dev_test/regen_sweep.py: DDIM with 1000-step dense
schedule, add_noise at training timestep N-1, head_start_step=1000-N so the
loop runs N denoising iterations through training timesteps [N-1, ..., 0].
SD 1.4 fp16, empty prompt, guidance 7.5, seed 1024.

Skips any output that already exists, so the run is safely resumable.
"""
...
def regen(image: Image.Image, pipe, n_steps: int, seed: int = 1024) -> Image.Image:
    device = pipe.device
    dtype = torch.float16
    ...
    nis = pipe.scheduler.config.num_train_timesteps  # 1000 for SD 1.4
    out = pipe(
        [""],
...
def main():
    ...
    ap.add_argument("--in-dir", default="valid_512")
    ap.add_argument("--out-dir", default="dev_test/regen_N020")
    ...
    for i, in_path in enumerate(inputs, 1):
        ...
        out = regen(src, pipe, n_steps=args.n_steps)
        out.save(out_path)
        p = psnr(src, out)
        psnrs.append(p)
