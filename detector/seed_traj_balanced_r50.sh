#!/bin/bash
# Same balanced trajectory sweep as seed_traj_balanced.sh but with ResNet-50.
# 4 seeds (44, 48, 53, 55) x constant LR, eval every 100 steps.
set -e
...
run() {
    local seed=$1
    local outdir=runs/seed_traj_bal_r50/s${seed}_constant
    echo
    ...
    $PY -u train_full.py --epochs 1 --batch 128 --image-size 384 --workers 8 \
        ...
        --out-dir $outdir \
        2>&1 | tee $outdir/full.log | grep --line-buffered -E '^  \[TRAJ\]|^epoch |saved best|^---|n=496|wrote'
}
...
echo
echo "==================== R50 BALANCED SWEEP SUMMARY ===================="
$PY -u <<'EOF'
import json, os, csv
print(f"{'run':<22s}  {'best_step':>9s}  {'test':>6s}  {'TPR':>6s}  {'TNR':>6s}  {'DiffDB':>7s}  {'COCO-edit':>9s}  {'COCO-nat-TNR':>13s}  {'balanced':>9s}")
for name in sorted(os.listdir("runs/seed_traj_bal_r50")):
    csv_path = f"runs/seed_traj_bal_r50/{name}/trajectory.csv"
